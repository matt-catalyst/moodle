...
function get_enrolled_sql(context $context, $withcapability = '', $groupid = 0, $onlyactive = false) {
    ...
    if ($withcapability) {
        list($joins[], $wheres[], $paramscap) = get_with_capability_join($context, $withcapability, $prefix.'u.id');
    }
 ...
 *
 * @param context $context
 * @params string $useridcolumn User id column used the calling query, e.g. u.id
 * @param bool $onlyactive consider only active enrolments in enabled plugins and time restrictions
 * @return array list($sql, $params)
 */
function get_enrolled_join(context $context, $useridcolumn, $onlyactive) {
    global $DB, $CFG;
    ...
    return array($joins, $wheres, $params);
}
...
function get_with_capability_sql($context, $capability) {
    global $DB, $CFG;
    ...
    list($joins, $wheres, $params) = get_with_capability_join($context, $capability, $prefix.'u.id');

    $sql = "SELECT DISTINCT {$prefix}u.id
            FROM {user} {$prefix}u
            $joins
            WHERE {$prefix}u.deleted = 0 AND $wheres
           ";
...
/**
 * Gets sql join for finding users with a capability in the given context
 *
 * @param context $context
 * @param string $capability
...
function get_with_capability_join($context, $capability, $useridcolumn) {
    global $DB, $CFG;
    ...
    if ($nobody) {
        ...
        if ($needed) {
            ...
            $joins[] = "JOIN {role_assignments} {$prefix}ra3 ON ({$prefix}ra3.userid = $useridcolumn AND {$prefix}ra3.roleid IN ($roleids) AND {$prefix}ra3.contextid IN ($ctxids))";
        }

        if ($prohibited) {
            $ctxids = implode(',', $contextids);
            $roleids = implode(',', array_keys($prohibited));
            $joins[] = "LEFT JOIN {role_assignments} {$prefix}ra4 ON ({$prefix}ra4.userid = $useridcolumn AND {$prefix}ra4.roleid IN ($roleids) AND {$prefix}ra4.contextid IN ($ctxids))";
